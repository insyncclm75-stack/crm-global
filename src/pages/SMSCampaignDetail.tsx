...
export default function SMSCampaignDetail() {
  ...
  const handleRetryFailed = async () => {
    try {
      // Update failed recipients to pending for retry
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `sms-campaign-${id}-recipients.csv`;
    a.click();
  };
            ...
            <TableBody>
              {recipients.map((recipient) => (
                <TableRow key={recipient.id}>
                  ...
                  <TableCell>{recipient.retry_count || 0}/{recipient.max_retries || 3}</TableCell>
